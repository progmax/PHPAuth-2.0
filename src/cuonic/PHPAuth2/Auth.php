...
class Auth
{
    ...
    */
    ...
    public function login($username, $password)
    {
        ...
        if ($this->isBlocked($ip)) return array_merge($return,array('code' => 0));
    ...
    public function getHash($string)
    {
        if (strnatcmp(phpversion(), '5.5.0') >= 0) {
            $enc = hash_pbkdf2(
                "SHA512",
                base64_encode(
                    str_rot13(hash("SHA512", str_rot13($this->config->salt_1 . $string . $this->config->salt_2)))
                ),
                $this->config->salt_3,
                50000,
                128
    ...
    */
    ...
    {
        if(strlen($action) == 0 || strlen($action) > 100 || strlen($info) == 0 || strlen($info) > 1000) return false;
    ...
    */
    ...
    private function addActivation($uid, $email)
    {
        ...
        if($this->isUserActivated($uid)) return false;
        ...
        if ($return) {
            ...
            $emailTemplate = new Localization\Handler(
                array('base_url' => $this->config->base_url,
                    'key' => $activekey
                ),
                $this->config->lang
            );
    ...
    */
    ...
    public function putLevel($hash, $uid, $level)
    {
        ...
        if ($admin_level >= $this->config->admin_level) return false;
